...
public class MemoryDAOTest {
	...
	@Test
	public void CanReturnCardsIfDifferent() throws Exception{
		MemoryAdapter game = dao.createNewGame();
		assertThat(game.getCurrentPlayer()).isNotNull();
		assertThat(game.getCurrentPlayer()).isEqualTo(0);
		game.returnCard(0);
		...
		game = dao.loadFromToken(game.getToken());
		assertThat(game.getCard(0).side).isEqualTo(Card.Side.VERSO);
		assertThat(game.getCard(1).side).isEqualTo(Card.Side.VERSO);
		assertThat(game.getCard(2).side).isEqualTo(Card.Side.VERSO);
